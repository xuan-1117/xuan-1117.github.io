...
### 简单rec
![](/assets/docs/match-wp/polar-range/img-001.png)
...
![](/assets/docs/match-wp/polar-range/img-002.png)
...
![](/assets/docs/match-wp/polar-range/img-005.png)

 注：header生成方式就是把header部分使用base64 url编码即可

![](/assets/docs/match-wp/polar-range/img-006.png)**payload**：jwt的主体信息，是一个JSON对象，它可以包含以下内容：
...
将JWT的值复制到**jwt.io**，查看jwt三部分的内容![](/assets/docs/match-wp/polar-range/img-007.png)

破解一下该JWT的密钥值，得到密钥SYSA，再根据数据包中给的提示，将payload的username改为admin，最后把新的jwt值复制到请求包中直接发包就成![](/assets/docs/match-wp/polar-range/img-008.png)

![](/assets/docs/match-wp/polar-range/img-009.png)

![](/assets/docs/match-wp/polar-range/img-010.png)
...
![](/assets/docs/match-wp/polar-range/img-011.png)

$$

![](/assets/docs/match-wp/polar-range/img-012.png)
...
![](/assets/docs/match-wp/polar-range/img-013.png)

### 签到题
![](/assets/docs/match-wp/polar-range/img-014.png)
...
### Session文件包含
进题随便输入一个数据，然后进到/action.php。用php伪协议读取action.php的源码，我去读取了，源码如下![](/assets/docs/match-wp/polar-range/img-015.png)

session包含。我们在name中上传一句话木马，并且在file中包含这个文件就可以了。这个文件的目录一般在tmp中，而且命名为：**sess_PHPSESSID，而这个sessid的位置，一般在cookie里面储存：**![](/assets/docs/match-wp/polar-range/img-016.png)
...
![](/assets/docs/match-wp/polar-range/img-017.png)
...
![](/assets/docs/match-wp/polar-range/img-018.png)
...
第二种：![](/assets/docs/match-wp/polar-range/img-019.png)

PHP反序列化测试

![](/assets/docs/match-wp/polar-range/img-020.png)
...
![](/assets/docs/match-wp/polar-range/img-021.png)

Easy.__wakeup -> Evil.__toString

![](/assets/docs/match-wp/polar-range/img-022.png)

审计

![](/assets/docs/match-wp/polar-range/img-023.png)
...
认为是相同的![](/assets/docs/match-wp/polar-range/img-024.png)
...
### 反序列化
![](/assets/docs/match-wp/polar-range/img-025.png)

![](/assets/docs/match-wp/polar-range/img-026.png)
...
SSRF 形成的原因大都是由于服务端提供了从其他服务器应用获取数据的功能且没有对目标地址做过滤与限制。![](/assets/docs/match-wp/polar-range/img-027.png)
...
gopher:// 分布式文档传递服务，可使用gopherus生成payload![](/assets/docs/match-wp/polar-range/img-028.png)
...
![](/assets/docs/match-wp/polar-range/img-029.png)
...
![](/assets/docs/match-wp/polar-range/img-030.png)

url二次编码

![](/assets/docs/match-wp/polar-range/img-031.png)

### <font style="color:rgba(0, 0, 0, 0.85);">ezphp</font>
扫目录发现robots

![](/assets/docs/match-wp/polar-range/img-032.png)

![](/assets/docs/match-wp/polar-range/img-033.png)
